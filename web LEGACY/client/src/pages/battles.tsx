...
export default function Battles() {
  ...
  const { data: userBattles, isLoading: userBattlesLoading } = useQuery<RecentBattle[]>({
    queryKey: ['/api/battles/user', user?.id],
    enabled: !!user?.id,
  });

  const { data: recentBattles, isLoading: recentBattlesLoading } = useQuery<RecentBattle[]>({
  ...
  const pvpBattles = userBattles?.filter(battle => battle.battleType === 'pvp') || [];
  const dungeonBattles = userBattles?.filter(battle => battle.battleType === 'dungeon') || [];
