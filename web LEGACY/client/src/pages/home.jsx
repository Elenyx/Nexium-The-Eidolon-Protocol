...
export default function Home() {
    ...
    }), recentBattles = _b.data, battlesLoading = _b.isLoading;
    ...
    return (<main className="container mx-auto px-4 py-8">
        ...
        <div className="relative bg-gradient-to-br from-secondary via-primary to-accent rounded-2xl p-12 mb-8 overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button asChild className="bg-accent hover:bg-accent/90 text-accent-foreground px-8 py-3 text-lg" data-testid="hero-start-journey">
                <Link href="/dashboard">Start Your Journey</Link>
              </Button>
              <Button variant="outline" className="border-2 border-primary-foreground text-primary-foreground hover:bg-primary-foreground hover:text-accent px-8 py-3 text-lg" data-testid="hero-join-discord">
                Join Our Discord
              </Button>
            </div>
          </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                {statsLoading ? (<Skeleton className="h-8 w-20"/>) : (<p className="text-2xl font-bold text-foreground" data-testid="stat-total-players">
                    {(stats === null || stats === void 0 ? void 0 : stats.totalPlayers.toLocaleString()) || '0'}
                  </p>)}
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                {statsLoading ? (<Skeleton className="h-8 w-20"/>) : (<p className="text-2xl font-bold text-foreground" data-testid="stat-active-battles">
                    {(stats === null || stats === void 0 ? void 0 : stats.activeBattles.toLocaleString()) || '0'}
                  </p>)}
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                {statsLoading ? (<Skeleton className="h-8 w-20"/>) : (<p className="text-2xl font-bold text-foreground" data-testid="stat-characters-collected">
                    {(stats === null || stats === void 0 ? void 0 : stats.charactersCollected.toLocaleString()) || '0'}
                  </p>)}
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                {statsLoading ? (<Skeleton className="h-8 w-20"/>) : (<p className="text-2xl font-bold text-foreground" data-testid="stat-territories-claimed">
                    {(stats === null || stats === void 0 ? void 0 : stats.territoriesClaimed.toLocaleString()) || '0'}
                  </p>)}
              </div>
      ...
      <section className="mb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {featuredCharacters.map(function (character) { return (<CharacterCard key={character.id} character={character}/>); })}
        </div>
      </section>
      ...
      <section className="mb-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {battlesLoading ? (Array.from({ length: 4 }).map(function (_, i) { return (<Card key={i} className="battle-card">
                <CardContent className="p-4">
                  <Skeleton className="h-20 w-full"/>
                </CardContent>
              </Card>); })) : recentBattles && recentBattles.length > 0 ? (recentBattles.slice(0, 4).map(function (battle) { return (<BattleCard key={battle.id} battle={battle}/>); })) : (<div className="col-span-2 text-center py-8 text-muted-foreground">
